            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header clearfix">
                        <h2 class="float-left">Tenants Details</h2>
                        <a href="tenants-create.php" class="btn btn-success float-right">Add New Record</a>
                        <a href="tenants-index.php" class="btn btn-info float-right mr-2">Reset View</a>
                        <a href="../index.php" class="btn btn-secondary float-right mr-2">Back</a>
                    </div>

                    <div class="form-row">
                        <form action="tenants-index.php" method="get">
                        <div class="col">
                    ...
                    $total_pages_sql = "SELECT COUNT(*) FROM tenants";
                    $result = mysqli_query($link,$total_pages_sql);
                    $total_rows = mysqli_fetch_array($result)[0];
                    $total_pages = ceil($total_rows / $no_of_records_per_page);

                    //Column sorting on column name
                    $orderBy = array('First_name', 'Last_name', 'Phone');
                    $order = 'Tenant_id';
                    ...
                    $sql = "SELECT * FROM tenants ORDER BY $order $sort LIMIT $offset, $no_of_records_per_page";
                    $count_pages = "SELECT * FROM tenants";


                    if(!empty($_GET['search'])) {
                        $search = ($_GET['search']);
                        $sql = "SELECT * FROM tenants
                            WHERE CONCAT_WS (First_name,Last_name,Phone)
                            LIKE '%$search%'
                            ORDER BY $order $sort
                        ...
                        if(mysqli_num_rows($result) > 0){
                            ...
                            echo "<table class='table table-bordered table-striped'>";
                                ...
                                while($row = mysqli_fetch_array($result)){
                                    echo "<tr>";
                                    echo "<td>" . $row['First_name'] . "</td>";echo "<td>" . $row['Last_name'] . "</td>";echo "<td>" . $row['Phone'] . "</td>";
                                        echo "<td>";
                                            echo "<a href='tenants-read.php?Tenant_id=". $row['Tenant_id'] ."' title='View Record' data-toggle='tooltip'><i class='far fa-eye'></i></a>";
                                            echo "<a href='tenants-update.php?Tenant_id=". $row['Tenant_id'] ."' title='Update Record' data-toggle='tooltip'><i class='far fa-edit'></i></a>";
                                            echo "<a href='tenants-delete.php?Tenant_id=". $row['Tenant_id'] ."' title='Delete Record' data-toggle='tooltip'><i class='far fa-trash-alt'></i></a>";
                                        echo "</td>";
                                    echo "</tr>";
                                }
                                echo "</tbody>";
